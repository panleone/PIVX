...
CallResult<CTxDestination> CWallet::getNewAddress(const std::string& addressLabel, const std::string purpose, const CChainParams::Base58Type addrType)
{
    LOCK(cs_wallet);
...
bool CWallet::EncryptWallet(const SecureString& strWalletPassphrase)
{
...

    {
        ...
        // if we are using HD, empty the keypool and generate new keys
...
static void ApproximateBestSubset(const std::vector<std::pair<CAmount, std::pair<const CWalletTx*, unsigned int> > >& vValue, const CAmount& nTotalLower, const CAmount& nTargetValue, std::vector<char>& vfBest, CAmount& nBest, int iterations = 1000)
{
...
bool CWallet::FundTransaction(CMutableTransaction& tx, CAmount& nFeeRet, bool overrideEstimatedFeeRate, const CFeeRate& specificFeeRate, int& nChangePosInOut, std::string& strFailReason, bool includeWatching, bool lockUnspents, const std::set<int>& setSubtractFeeFromOutputs, const CTxDestination& destChange)
{
    ...
    const int nExtraSize = tx.isSaplingVersion() ?
                               (int)(GetSerializeSize(tx.sapData) + GetSerializeSize(tx.extraPayload)) :
                               0;
